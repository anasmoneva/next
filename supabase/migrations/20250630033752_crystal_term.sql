/*
  ...
  1. New Tables
    ...
    - `categories` - Store registration categories with fees and details
    - `registrations` - Store user registrations with status tracking
    - `admins` - Store admin users with role-based access

  2. Security
    - Enable RLS on all tables
    - Add policies for public access to necessary data
    - Add policies for admin authentication

  3. Sample Data
    - Insert Kerala panchayaths including Amarambalam
    - Insert registration categories as specified
    - Insert admin users with specified credentials
*/
...
-- Add unique constraint for panchayaths
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Add unique constraint for registrations
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Add unique constraint for admins
DO $$
BEGIN
  IF NOT EXISTS (
